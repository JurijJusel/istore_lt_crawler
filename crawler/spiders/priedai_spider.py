...
class PriedaiSpider(scrapy.Spider):
    name = "priedai_spider"
    allowed_domains = ['istore.lt']
    url = "https://istore.lt/apple-priedai"

    custom_settings = {
        'LOG_FILE': 'logs/priedai.log',
        'FEEDS': {
            'data/priedai_output.jsonl' : {
                'format': 'jsonlines',
                'encoding': 'utf8',
                'store_empty': False,
                'overwrite': True
    ...
    def closed(self, reason):
        print("\n\n--- istore.lt priedai products Crawl Finished ---")
